    ...
    def run(self):  
        ...
        try:  
            while True:  
                ...
                # Exit if any window is closed via the X button  
                if (cv2.getWindowProperty("Original Video", cv2.WND_PROP_VISIBLE) < 1 or  
                    cv2.getWindowProperty("Canny Edges", cv2.WND_PROP_VISIBLE) < 1 or  
                    cv2.getWindowProperty("Contour Detection", cv2.WND_PROP_VISIBLE) < 1):  
                    break  
        finally:  
            # Release resources  
            self.cleanup()  
...
if __name__ == "__main__":  
    import argparse
    parser = argparse.ArgumentParser(description="OpenCV Contour Detection Demo")
    parser.add_argument("-c", "--camera", type=int, default=0, help="Camera index (default: 0)")
    args = parser.parse_args()

    try:  
        detector = CameraContourDetector(camera_index=args.camera)  
        detector.run()  
